...
<template>
    <Sortable v-model="module.lessons" item-key="id" tag="ul" group="lessons" handle=".handle" @end="$emit('end')">
        <template #item="{ element: lesson }">
            <li
                ...
                <div class="flex-1 max-w-full flex items-center gap-4">
                    <div
                        class="text-slate-400 hover:text-slate-950 handle cursor-move opacity-0 group-hover:opacity-100 duration-300">
                    ...
                    <span v-if="lesson.publishAt" class="text-slate-400 text-xs hidden lg:flex items-center gap-2">
                ...
                <div class="flex gap-2 items-center justify-end">
                    <TagSelector v-model="lesson.accessLevelId" :options="organization.accessLevels"
    ...
    <FormDialog resource="Lesson" v-model:open="dialog.isOpen" :editing="dialog.resource?.id"
        :processing="form.processing" @create="onSubmit" @update="onSubmit">
        <FormInput ref="dialogFocusEl" label="Name" v-model="form.name" :error="form.errors.name"
            placeholder="My Cool Lesson" />

        <FormInput type="group" label="Publish At" :error="form.errors.publishAtDate || form.errors.publishAtTime">
